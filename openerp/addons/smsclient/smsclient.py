...
class partner_sms_send(orm.Model):
    ...
    def _default_get_mobile(self, cr, uid, fields, context=None):
        if context is None:
            context = {}
        partner_pool = self.pool.get('tapplicant')
    ...
    _columns = {
        'mobile_to': fields.char('To', size=520, required=True),
        'app_id': fields.char('API ID', size=520),
        'user': fields.char('Login', size=520),
        'password': fields.char('Password', size=520),
...
class SMSClient(orm.Model):
    ...
    _columns = {
        'name': fields.char('Gateway Name', size=520, required=True),
        'url': fields.char('Gateway URL', size=520,
        ...
        'code': fields.char('Verification Code', size=520),
...
class SMSQueue(orm.Model):
    ...
    _columns = {
        'name': fields.text('SMS Request', size=520,
            required=True, readonly=True,
            states={'draft': [('readonly', False)]}),
        'msg': fields.text('SMS Text', size=520,
            required=True, readonly=True,
            states={'draft': [('readonly', False)]}),
        'mobile': fields.char('Mobile No', size=520,
            required=True, readonly=True,
            states={'draft': [('readonly', False)]}),
        ...
        'error': fields.text('Last Error', size=520,
            readonly=True,
            states={'draft': [('readonly', False)]}),
...
class Properties(orm.Model):
    ...
    _columns = {
        'name': fields.char('Property name', size=520,
             help='Name of the property whom appear on the URL'),
        'value': fields.char('Property value', size=520,
...
class HistoryLine(orm.Model):
    ...
    _columns = {
        'name': fields.char('Description', size=520, required=True, readonly=True),
        'date_create': fields.datetime('Date', readonly=True),
        ...
        'sms': fields.text('SMS', size=520, readonly=True),
    }
